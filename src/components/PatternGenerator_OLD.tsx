...
export default function PatternGenerator() {
  ...
  const patternCellsRef = useRef<typeof patternCells>({}); // Ref for immediate access to patternCells
  ...
  // Keep patternCellsRef in sync
  useEffect(() => {
    patternCellsRef.current = patternCells;
  }, [patternCells]);
    ...
    async function generatePatternAsync() {
      try {
        ...
        if (shouldRegenerateRandom) {
          ...
          // Copy from ref for immediate access (no waiting for React state)
          Object.assign(patternCellsData, patternCellsRef.current);
        }
        ...
        // Update BOTH ref and state (ref for immediate access, state for React render)
        patternCellsRef.current = patternCellsData;
...
// Regenerate SVG whenever patternCells changes (from movement, rotation, etc)
useEffect(() => {
  let cancelled = false;
  
  async function regenerateSVG() {
    try {
      console.log('[PatternGenerator] Regenerating SVG from patternCells');
      
      // Validate config
      if (!config.containerSize || !config.shapes?.length || !config.colors?.length) {
        return;
      }
      
      const gridSize = config.gridSize || 4;
      
      // Build cells array directly from patternCells state
      const cells: Array<{row: number, col: number, shapeId: string, bgColorIndex: number, fgColorIndex: number}> = [];
      ...
      console.log('[PatternGenerator] Regenerating SVG for', cells.length, 'cells');
      
      // Generate pattern and SVG
      const elements = await generatePattern(config, cells);
      if (cancelled) return;
      ...
      console.error('[PatternGenerator] SVG regeneration error:', err);
    }
  }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (selectedCells.size > 0 && (
        ...
        // Perform MOVE (not swap) using patternCells (single source of truth)
        // Use ref for immediate, synchronous access to current cell data
        const newCells = { ...patternCellsRef.current };
        
        console.log('[DEBUG] Moving cells:', Array.from(cellMoves.entries()).map(([from, to]) => `${from} -> ${to}`).join(', '));
        
        // Step 1: Take a snapshot of all cells we're moving
        const movingCells = new Map<string, typeof newCells[string]>();
        for (const [oldKey, newKey] of cellMoves.entries()) {
          movingCells.set(oldKey, newCells[oldKey]);
          console.log(`[DEBUG] Snapshot: ${oldKey} = ${newCells[oldKey]?.shapeId || 'null'}`);
        }
        
        // Step 2: Clear the source cells (only if they're not also destinations)
        const destinationKeys = new Set(cellMoves.values());
        for (const oldKey of cellMoves.keys()) {
          if (!destinationKeys.has(oldKey)) {
            newCells[oldKey] = null;
            console.log(`[DEBUG] Cleared source: ${oldKey}`);
          }
        }
        
        // Step 3: Place cells at their new destinations
        for (const [oldKey, newKey] of cellMoves.entries()) {
          const cellData = movingCells.get(oldKey);
          newCells[newKey] = cellData ?? null;
          console.log(`[DEBUG] Moved: ${oldKey} -> ${newKey}, shape: ${cellData?.shapeId || 'null'}`);
        }
        
        // Update BOTH ref and state (ref for immediate access, state for React render)
        patternCellsRef.current = newCells;
        setPatternCells(newCells);
        console.log('[DEBUG] Updated patternCells with', Object.keys(newCells).length, 'cells');
        
        // Trigger animation
        const swapPairs = Array.from(cellMoves.entries()).map(([from, to]) => ({ from, to }));
